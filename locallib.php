 ...
 */
function block_intuitel_check_access() {
    global $CFG;
...
// 	 	    $remote_addr = '127.0.0.1';
        if (block_intuitel_ipv4_match($remote_addr, $ip))
            return true;
        if (block_intuitel_ipv6_match($remote_addr, $ip))
            return true;
    }
 ...
 * @return boolean
 */
function block_intuitel_ipv4_match($ip, $cidrnet) {
    if ($ip == '::1')
        $ip = '127.0.0.1';
 ...
 * @return boolean
 */
function block_intuitel_ipv6_match($ip_string, $cidrnet) {
    $parts = explode('/', $cidrnet);
    if (count($parts) != 2) {
        return false;
    } else {
    ...
    $binaryip = block_intuitel_inet_to_bits($ip);
    $binarynet = block_intuitel_inet_to_bits($net);
...
function block_intuitel_inet_to_bits($inet) {
...
function block_intuitel_submit_to_intuitel($xml, $aditional_params = array()) {
    ...
    $return = $rest_client->post($intuitelEndPoint, $xml,
    ...
    try {
        ...
        if (count($intuitel_elements->Learner->Tug) > 0) {
            ...
            foreach ($intuitel_elements->Learner->Tug as $tug) {
                ...
                IntuitelController::addTUGInmediateResponse($response, IntuitelXMLSerializer::get_required_attribute($tug, 'uId'),
                        IntuitelXMLSerializer::get_required_attribute($tug, 'mId'), "OK");
            }
            $xml = $response->asXML();
        ...
        if (count($intuitel_elements->Learner->Lore) > 0) {
            ...
            foreach ($intuitel_elements->Learner->Lore as $lore) {
                ...
                $xml = IntuitelController::addLOREInmediateResponse($response,
                                ...
                                IntuitelXMLSerializer::get_required_attribute($lore, 'mId'), "OK");
            }
            $xml = $response->asXML();
            block_intuitel_submit_to_intuitel($xml);
        }
...
function block_intuitel_get_service_endpoint() {
    global $CFG;
...
function block_intuitel_getMediaType($content) {
    // add an i to the end of the pattern so no upper and lower case is distinguised
    $mediatype = '';
    if (preg_match('/<img src/i', $content) == 1)
        $mediatype = 'image';
    return $mediatype;
}
 ...
 */
function block_intuitel_get_moodle_section($sectionid) {
    global $DB;
 ...
 */
function block_intuitel_get_cm($cmid) {
    global $DB;
 ...
 */
function block_intuitel_get_cm_type($cmid) {
    global $DB;
 ...
 */
function block_intuitel_get_parent_lang($lang) {
    ...
    if ($completion->is_enabled() > 0) {
        ...
        if (!empty($criteria)) {  //if criteria has been stablished for completing the course (in another case there is no course completion tracking)
            ...
            $timecompleted = $DB->get_field('course_completions', 'timecompleted',
                    array('userid' => $native_user_id, 'course' => $lo_id));
            if ($timecompleted == false)
                $completion_status = 0; //course has not been completed
            else
 ...
 */
function block_intuitel_get_access_status($cmid, $userid) {
 ...
 */
function block_intuitel_get_access_status_course($courseid, $native_user_id) {
...
function block_intuitel_get_grade_info(\cm_info $coursemodule_info, $userid) {
    ...
    if ($gradable) {
        if (grade_is_user_graded_in_activity($coursemodule_info, $userid)) {
            ...
            $grade_complete_info = \grade_get_grades($coursemodule_info->course, 'mod', $coursemodule_info->modname,
                    $coursemodule_info->instance, $userid);
 ...
 */
function block_intuitel_get_online_users(array $native_userids) {
    global $DB, $CFG;
    $online_users = array();
    ...
    foreach ($intuitel_elements->Learner->Tug as $tug) {
        ...
        if ($mtype == '1') { //Simple message, not important
            $html .= block_intuitel_write_form_start($mid, $courseid) .
                    '<p>' . $tug_mdata . '</p>' .
                    block_intuitel_write_form_end($mid, false);
            ...
            $html.=
                    block_intuitel_write_form_start($mid, $courseid) .
                    '<p>' . $tug_mdata . '</p>
			...
			<input type="RADIO" NAME="YesNo" value="No" >' . get_string('no', 'moodle') . '<br>' .
                    block_intuitel_write_form_end($mid);
            $html = block_intuitel_add_fadeIn_jscript($mid, $html);
            $html = block_intuitel_add_popup_notification($mid, $html);
 ...
 */
function block_intuitel_filter_out_trailing_lang_mark($tug_mdata) {
 ...
 */
function block_intuitel_change_select_into_radio($tug_mdata) {
    $matches = array();
...
function block_intuitel_write_form_start($mid, $courseid) {
    global $CFG;
    $wwwroot = $CFG->wwwroot;
    return <<<html
...
function block_intuitel_write_form_end($mid, $showSubmit = true) {
    $html = <<<html
...
function block_intuitel_add_fadeIn_jscript($mid, $html) {
    return $html;
    $jscript = <<<code
 ...
 */
function block_intuitel_add_printing_code($mid, $html) {
    global $OUTPUT;
...
function block_intuitel_add_popup_notification($mid, $html) {
    global $PAGE;
    $url = '';
 ...
 */
function block_intuitel_get_input_message($methods = 'POST,GET') {
 ...
 */
function block_intuitel_disable_moodle_page_exception_handler() {
    ob_start();
    set_exception_handler('block_intuitel_exception_handler');
    global $PAGE;
    $PAGE->set_context(context_system::instance());
}

function block_intuitel_exception_handler($exception) {
