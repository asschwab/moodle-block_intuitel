...
xml;
         ...
         */
        if ($previousKOEvent != null && $previousKOEvent->duration < $this->enough_time && $previousKOType != 'course' && $previousKOType != 'forum' && $previousKOType != 'folder' && (!$previous_use_data['grade'] || !$previous_use_data['grade'] > $previous_use_data['grademax'] * 2 / 3) // Do not warn if grade is already high
        ) {
            $mid = Intuitel::getIDFactory()->getNewMessageUUID();
            $a = new \stdClass();
...
xml;
         ...
         */
        $previousSiblingCount = $currentKO && $currentKO->hasPrecedingSib && array_key_exists($currentKO->hasPrecedingSib->id,
                        $revisits) ? $revisits[$currentKO->hasPrecedingSib->id] : 0;
