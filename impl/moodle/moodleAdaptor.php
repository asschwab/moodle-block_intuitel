...
class moodleAdaptor extends IntuitelAdaptor {
	 ...
	 */
	function getLearnerUpdateData(array $native_user_ids=null, CourseLO $course=null, $from=null, $to=null,$filter_offline_users=true)
	{
	    global $DB;
	    ...
	    global $DB;
	     ...
	     if(!empty($native_activeuserids)){
	         ...
	         foreach($events as $event){
	             ...
	             elseif ($event->target=='course_module' && $event->action=='viewed')
	             {  //the visited LO is a module
	                 $loID=Intuitel::getIDFactory()->getLoIdfromId('module', $event->contextinstanceid);
	                 $visitEvents[$userId->id()][$event->id]=new VisitEvent($userId,$loID,$event->timecreated);
	             }
	         }
	         if($to==null)
	             $this->markLearnerUpdatePollTime($native_activeuserids,time());  //mark polltimes to avoid re-poll same information
	         else
	...
	function getGuessedFactory($data)
	{
		...
		elseif (isset ( $data->section )) 		// Moodle section
		{
			$type = 'section';
		}
